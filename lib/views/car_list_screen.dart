...
// print('kkkkkkkkkkkkkkkkkkkkkkkkkkkkk$url');
...
//     });
// print('kkkkkkkkkkkkkkkkkkkkkkkkkkkkkppppp');
...
class _CarStatusScreenState extends State<CarStatusScreen>
  ...
  // Date and time selection (actual applied filters)
  DateTime? startDate;
  DateTime? endDate;
  TimeOfDay? fromTime;
  TimeOfDay? toTime;
  
  // Temporary variables for date/time before applying
  ...
  TimeOfDay tempToTime = TimeOfDay.now();
  ...
  void initState() {
    ...
    _tabController = TabController(length: 2, vsync: this);
    _tabController.addListener(_handleTabChange);
    // Load initial data WITHOUT any date/time filters
    _fetchAllData();
  }
  ...
  Future<void> _fetchAvailableCars() async {
    ...
    try {
      // Build URL - start with base URL without filters
      String url = 'https://varahibackend.varahiselfdrivecars.com/api/car/carsbystatus?status=available';
      
      // ONLY add date/time filters if they have been applied (not null)
      if (startDate != null && endDate != null && fromTime != null && toTime != null) {
        url += '&startDate=${_formatDate(startDate!)}&endDate=${_formatDate(endDate!)}&fromTime=${_formatTime(fromTime!)}&toTime=${_formatTime(toTime!)}';
        print('Fetching available cars WITH filters: $url');
      } else {
        print('Fetching available cars WITHOUT filters: $url');
      }
  ...
  Future<void> _fetchBookedCars() async {
    ...
    try {
      // Build URL - start with base URL without filters
      String url = 'https://varahibackend.varahiselfdrivecars.com/api/car/carsbystatus?status=booked';
      
      // ONLY add date/time filters if they have been applied (not null)
      if (startDate != null && endDate != null && fromTime != null && toTime != null) {
        url += '&startDate=${_formatDate(startDate!)}&endDate=${_formatDate(endDate!)}&fromTime=${_formatTime(fromTime!)}&toTime=${_formatTime(toTime!)}';
        print('Fetching booked cars WITH filters: $url');
      } else {
        print('Fetching booked cars WITHOUT filters: $url');
      }
  ...
  // Apply filters - this is the only method that triggers API calls with filters
  Future<void> _applyFilters() async {
    setState(() {
      // Apply the temporary values to actual values
  ...
  // Clear filters - remove all date/time filters
  Future<void> _clearFilters() async {
    setState(() {
      // Set actual filters to null (no filters)
      startDate = null;
      endDate = null;
      fromTime = null;
      toTime = null;
      ...
      tempEndDate = DateTime.now().add(const Duration(days: 1));
      tempFromTime = TimeOfDay.now();
      tempToTime = TimeOfDay.now();
    });
  ...
  Widget build(BuildContext context) {
    ...
    final bool hasActiveFilters = startDate != null && endDate != null && fromTime != null && toTime != null;
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // Date and Time Selection Bar with Apply Button
          Container(
            padding: const EdgeInsets.all(12),
            color: Colors.grey[100],
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Expanded(
                      child: _buildDateButton(
                        'Start: ${DateFormat('dd MMM yyyy').format(tempStartDate)}',
                        Icons.event,
                        _selectStartDate,
                      ),
                    ),
                    ...
                    Expanded(
                      child: _buildDateButton(
                        'End: ${DateFormat('dd MMM yyyy').format(tempEndDate)}',
                        Icons.event_note,
                        _selectEndDate,
                      ),
                    ),
                ...
                Row(
                  children: [
                    Expanded(
                      child: _buildTimeButton(
                        'From: ${tempFromTime.format(context)}',
                        Icons.access_time,
                        _selectFromTime,
                      ),
                    ),
                  ...
                  children: [
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: isApplying ? null : _applyFilters,
                        icon: isApplying
                            ? const SizedBox(
                                width: 20,
                                height: 20,
                        ...
                        label: Text(isApplying ? 'Applying...' : 'Apply Filters'),
          ...
          // Show active filters info (only when filters are applied)
          if (hasActiveFilters)
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Filtered: ${DateFormat('dd MMM').format(startDate!)} - ${DateFormat('dd MMM').format(endDate!)} | ${fromTime!.format(context)} - ${toTime!.format(context)}',
